...
void rtfmm::LaplaceKernel::direct(Bodies3 &bs_src, Bodies3 &bs_tar, int images,
                                  real cycle)
{
    vec4d zero(0);
    vec4d one(1);
...
void rtfmm::LaplaceKernel::p2p(Bodies3 &bs_src, Bodies3 &bs_tar,
                               Cell3 &cell_src, Cell3 &cell_tar, vec3r offset,
                               int use_simd)
{
    if (!use_simd)
    {
    ...
    else
    {
        ...
        for (int j = 0; j < cell_tar.brange.number; j += 4)
        {
            ...
            if (padding <= 0)
            {
                xj = vec4d(btar->x[0], (btar + 1)->x[0], (btar + 2)->x[0],
                           (btar + 3)->x[0]);
                yj = vec4d(btar->x[1], (btar + 1)->x[1], (btar + 2)->x[1],
                           (btar + 3)->x[1]);
                zj = vec4d(btar->x[2], (btar + 1)->x[2], (btar + 2)->x[2],
                           (btar + 3)->x[2]);
            }
            else if (padding == 1)
            {
...
void rtfmm::LaplaceKernel::p2p_1toN_128(
    Bodies3 &bs_src, Bodies3 &bs_tar, Cells3 &cs,
    std::vector<std::pair<int, vec3r>> &p2ps, Cell3 &cell_tar)
{
...
void rtfmm::LaplaceKernel::p2p_1toN_256(
    Cells3 &cs, std::vector<std::pair<int, vec3r>> &p2ps, Cell3 &cell_tar)
{
    vec4d zero(0);
    vec4d one(1);
    vec4d dx, dy, dz, qi, invr, f, qinvr;
    for (int j = 0; j < cell_tar.bodies.size(); j += 4)
    {
        Body3 *btar = &cell_tar.bodies[j];
        real *wtar = &cell_tar.weights[j];
        vec4d xj(0), yj(0), zj(0);
        int padding = j + 4 - cell_tar.bodies.size();
        padding = padding <= 0 ? 0 : padding;
        for (int k = 0; k < 4 - padding; k++)
        {
            xj[k] = (btar + k)->x[0];
        ...
        for (int si = 0; si < p2ps.size(); si++)
        {
            ...
            {
                Body3 &bsrc = cell_src.bodies[i];
                real &wsrc = cell_src.weights[i];
                vec3r bsrc_xi = bsrc.x + offset;
                dx = vec4d(bsrc_xi[0]);
                dy = vec4d(bsrc_xi[1]);
                dz = vec4d(bsrc_xi[2]);
        ...
        for (int k = 0; k < 4 - padding; k++)
        {
            if ((btar + k)->idx == 0)
            {
                printf("P2P body %d, cell %d, r=%.4f, p2p_size=%ld, w*p=%.12f "
                       "* %.12f = %.12f\n",
                       0, cell_tar.idx, cell_tar.r, p2ps.size(), *(wtar + k),
                       pj[k], *(wtar + k) * pj[k]);
            }
...
void rtfmm::LaplaceKernel::p2m(int P, Bodies3 &bs_src, Cell3 &cell_src)
{
    ...
    std::vector<vec3r> x_check =
        get_surface_points(P, cell_src.r * 2.95, cell_src.x);
    ...
    std::vector<rtfmm::vec3r> x_equiv =
        get_surface_points(P, cell_src.r * 1.05, cell_src.x);
...
void rtfmm::LaplaceKernel::p2m_precompute(int P, Cell3 &cell_src)
{
    ...
    Matrix &p_check =
        cell_src.q_equiv; // use q_equiv as p_check_buffer temporarily
    vec4d zero(0), one(1);
    ...
    for (int j = 0; j < num_surf; j += 4)
    {
        ...
        for (int i = 0; i < cell_src.bodies.size(); i++)
        {
            ...
            {
                std::cout << "P2M " << cell_src << " has body0, q=" << bi->q
                          << ",weight = " << *wi << std::endl;
            }
            dx = xj - vec4d(bi->x[0]);
            dy = yj - vec4d(bi->x[1]);
...
void rtfmm::LaplaceKernel::m2m(int P, Cell3 &cell_parent, Cells3 &cs)
{
    ...
    std::vector<rtfmm::vec3r> x_equiv_parent =
        get_surface_points(P, cell_parent.r * 1.05, cell_parent.x);
    std::vector<rtfmm::vec3r> x_check_parent =
        get_surface_points(P, cell_parent.r * 2.95, cell_parent.x);
    ...
    for (int i = 0; i < cell_parent.crange.number; i++)
    {
        ...
        std::vector<rtfmm::vec3r> x_equiv_child =
            get_surface_points(P, cell_child.r * 1.05, cell_child.x);
        ...
        Matrix q_equiv_parent =
            linear_equation_system_svd(pe2pc, p_check_parent);
...
void rtfmm::LaplaceKernel::m2m_precompute(int P, Cell3 &cell_parent, Cells3 &cs)
{
    ...
    {
        Cell3 &cell_child = cs[cell_parent.crange.offset + i];
        mat_vec_mul(matrix_m2m[cell_child.octant], cell_child.q_equiv,
                    q_equiv_parent);
        mat_mat_increment(cell_parent.q_equiv, q_equiv_parent);
    }
}

void rtfmm::LaplaceKernel::m2m_img(int P, Cell3 &cell_parent, Cells3 &cs,
                                   real cycle)
{
    /* parent equiv to parent check matrix */
    std::vector<rtfmm::vec3r> x_equiv_parent =
    ...
    for (int pz = -1; pz <= 1; pz++)
    {
        for (int py = -1; py <= 1; py++)
        {
            for (int px = -1; px <= 1; px++)
            {
                ...
                /* child equivalent to parent check */
                std::vector<rtfmm::vec3r> x_equiv_child = get_surface_points(
                    P, cell_child.r * 1.05,
                    cell_child.x + vec3r(px, py, pz) * cycle);
                Matrix ce2pc = get_p2p_matrix(x_equiv_child, x_check_parent);
                Matrix p_check_parent = mat_vec_mul(ce2pc, cell_child.q_equiv);
                ...
                Matrix q_equiv_parent =
                    linear_equation_system_svd(pe2pc, p_check_parent);
                cell_parent.q_equiv =
                    mat_mat_add(cell_parent.q_equiv, q_equiv_parent);
            }
        }
    }
}

void rtfmm::LaplaceKernel::m2m_img_precompute(int P, Cell3 &cell_parent,
                                              Cells3 &cs, real cycle)
{
    Cell3 &cell_child = cs[cell_parent.crange.offset];
    for (int octant = 0; octant < 27; octant++)
    {
        Matrix q_equiv_parent =
            mat_vec_mul(matrix_m2m_img[octant], cell_child.q_equiv);
        cell_parent.q_equiv = mat_mat_add(cell_parent.q_equiv, q_equiv_parent);
    }
}
                               ...
                               vec3r offset)
{
    ...
    std::vector<rtfmm::vec3r> x_equiv =
        get_surface_points(P, cell_src.r * 1.05, cell_src.x + offset);
    std::vector<rtfmm::vec3r> x_check =
        get_surface_points(P, cell_tar.r * 1.05, cell_tar.x);
    Matrix e2c = get_p2p_matrix(x_equiv, x_check);
...
void rtfmm::LaplaceKernel::m2l_fft(int P, Cell3 &cell_src, Cell3 &cell_tar,
                                   vec3r offset)
{
    ...
    std::vector<rtfmm::vec3r> grid =
        get_conv_grid(N, gsize, delta, relative_pos);
    std::vector<rtfmm::real> G = get_G_matrix(grid, N);
    std::map<int, rtfmm::vec3i> surf_conv_map = get_surface_conv_map(P);
    std::vector<rtfmm::real> Q =
        get_Q_matrix(cell_src.q_equiv, N, surf_conv_map);
    ...
    fftw_plan plan_G =
        fftw_plan_dft_r2c_3d(N, N, N, G.data(), Gk.data(), FFTW_ESTIMATE);
    fftw_plan plan_Q =
        fftw_plan_dft_r2c_3d(N, N, N, Q.data(), Qk.data(), FFTW_ESTIMATE);
    fftw_execute(plan_G);
    fftw_execute(plan_Q);
    ...
    fftw_plan plan_P = fftw_plan_dft_c2r_3d(N, N, N, pk.data(),
                                            p_fft_grid.data(), FFTW_ESTIMATE);
    fftw_execute(plan_P);
    for (int i = 0; i < N * N * N; i++)
    {
        p_fft_grid[i] /= N * N * N;
    ...
    {
        rtfmm::vec3i idx3 =
            surf_conv_map[i] + rtfmm::vec3i(P - 1, P - 1, P - 1);
...
void rtfmm::LaplaceKernel::m2l_fft_precompute_naive(
    int P, Cells3 &cs, PeriodicInteractionPairs &m2l_pairs)
{
    ...
    std::vector<std::vector<complexr>> Gk_all(num_fft,
                                              std::vector<complexr>(N3));
    std::vector<std::vector<complexr>> Qk_all(num_fft,
                                              std::vector<complexr>(N3));
    std::vector<std::vector<complexr>> pk_all(num_fft,
                                              std::vector<complexr>(N3));
    ...
    for (int i = 0; i < num_fft; i++)
    {
        ...
        std::vector<rtfmm::vec3r> x_equiv =
            get_surface_points(P, r, relative_pos);
        rtfmm::real delta = 2 * r / (P - 1);
        rtfmm::real gsize = r * 2;
        std::vector<rtfmm::vec3r> grid =
            get_conv_grid(N, gsize, delta, relative_pos);
        ...
        plans_G[i] = fftw_plan_dft_r2c_3d(
            N, N, N, G.data(), reinterpret_cast<fftw_complex *>(Gk.data()),
            FFTW_ESTIMATE);
        plans_Q[i] = fftw_plan_dft_r2c_3d(
            N, N, N, Q.data(), reinterpret_cast<fftw_complex *>(Qk.data()),
            FFTW_ESTIMATE);
        plans_P[i] = fftw_plan_dft_c2r_3d(
            N, N, N, reinterpret_cast<fftw_complex *>(pk.data()),
            p_fft_grid.data(), FFTW_ESTIMATE);
...
#pragma omp parallel for
    ...
    for (int i = 0; i < num_plan; i++)
    {
        ...
        for (int i = 0; i < surface_number; i++)
        {
            rtfmm::vec3i idx3 =
                surf_conv_map[i] + rtfmm::vec3i(P - 1, P - 1, P - 1);
            cell_tar.p_check.d[i] +=
                p_fft_grid[idx3[2] * N * N + idx3[1] * N + idx3[0]] /
                (N * N * N);
        }
    }
}

void rtfmm::LaplaceKernel::m2l_fft_precompute_advanced(
    int P, Cells3 &cs, PeriodicInteractionPairs &m2l_pairs)
{
    ...
    std::vector<std::vector<complexr>> Gk_all(num_fft,
                                              std::vector<complexr>(N3));
    std::vector<std::vector<complexr>> Qk_all(num_src,
                                              std::vector<complexr>(N3));
    std::vector<std::vector<complexr>> pk_all(num_fft,
                                              std::vector<complexr>(N3));
    TIME_END(init_mem);
...
#pragma omp parallel for
    ...
    fftw_plan plan_G = fftw_plan_dft_r2c_3d(
        N, N, N, G.data(), reinterpret_cast<fftw_complex *>(Gk.data()),
        FFTW_ESTIMATE);
    fftw_plan plan_Q = fftw_plan_dft_r2c_3d(
        N, N, N, Q.data(), reinterpret_cast<fftw_complex *>(Qk.data()),
        FFTW_ESTIMATE);
    fftw_plan plan_P = fftw_plan_dft_c2r_3d(
        N, N, N, reinterpret_cast<fftw_complex *>(pk.data()), p_fft_grid.data(),
        FFTW_ESTIMATE);
...
#pragma omp parallel for
    for (int i = 0; i < num_fft; i++)
    {
        std::vector<rtfmm::real> &G = G_all[i];
        std::vector<complexr> &Gk = Gk_all[i];
        fftw_execute_dft_r2c(plan_G, G.data(),
                             reinterpret_cast<fftw_complex *>(Gk.data()));
    }
    TIME_END(G);
    TIME_BEGIN(Q);
#pragma omp parallel for
    for (int i = 0; i < num_src; i++)
    {
        std::vector<rtfmm::real> &Q = Q_all[i];
        std::vector<complexr> &Qk = Qk_all[i];
        fftw_execute_dft_r2c(plan_Q, Q.data(),
                             reinterpret_cast<fftw_complex *>(Qk.data()));
    }
    TIME_END(Q);
...
#pragma omp parallel for
    ...
    {
        std::vector<complexr> &Gk = Gk_all[i];
        std::vector<complexr> &Qk =
            Qk_all[m2l_src_map[m2l_pairs[i].second.first]];
        std::vector<complexr> &pk = pk_all[i];
        std::vector<rtfmm::real> &p_fft_grid = p_all[i];
        ...
        fftw_execute_dft_c2r(plan_P,
                             reinterpret_cast<fftw_complex *>(pk.data()),
                             p_fft_grid.data());
    }
    TIME_END(P);
...
void rtfmm::LaplaceKernel::m2l_fft_precompute_advanced2(
    int P, Cells3 &cs, PeriodicInteractionMapM2L &m2l_map)
{
    if (verbose)
    {
...
#ifdef USE_MANY
    fftw_plan plan_Q =
        fftw_plan_many_dft_r2c(3, fft_size, batch_size, &Q_all[0], nullptr, 1,
                               N3, reinterpret_cast<fftw_complex *>(&Qk_all[0]),
                               nullptr, 1, N_freq, FFTW_ESTIMATE);
#pragma omp parallel for
    for (int batch_idx = 0; batch_idx < batch_num; batch_idx++)
    {
        ...
        fftw_execute_dft_r2c(plan_Q, Q,
                             reinterpret_cast<fftw_complex *>(&Qk[0]));
    }
#else
    fftw_plan plan_Q = fftw_plan_dft_r2c_3d(
        N, N, N, &Q_all[0], reinterpret_cast<fftw_complex *>(&Qk_all[0]),
        FFTW_ESTIMATE);
#pragma omp parallel for
    for (int i = 0; i < num_src; i++)
    {
...
#pragma omp parallel for schedule(dynamic)
    for (int i = 0; i < num_tar; i++)
    {
        ...
        for (int j = 0; j < m2l_len; j++)
        {
            ...
            vec3r rv = (cell_src.x + m2l->second[j].second - cell_tar.x) /
                       (cell_src.r * 2);
    ...
    TIME_BEGIN(down);
    fftw_plan plan_P = fftw_plan_dft_c2r_3d(
        N, N, N, reinterpret_cast<fftw_complex *>(&pk_all[0]), &p_all[0],
        FFTW_ESTIMATE);
#pragma omp parallel for
    for (int i = 0; i < num_tar; i++)
    {
        ...
        fftw_execute_dft_c2r(plan_P, reinterpret_cast<fftw_complex *>(pk),
                             p_fft_grid);
        auto m2l = m2l_map.begin();
        std::advance(m2l, i);
        Cell3 &cell_tar = cs[m2l->first];
        ...
        for (int i = 0; i < surface_number; i++)
        {
            rtfmm::vec3i idx3 =
                surf_conv_map[i] + rtfmm::vec3i(P - 1, P - 1, P - 1);
            cell_tar.p_check.d[i] +=
                p_fft_grid[idx3[2] * N * N + idx3[1] * N + idx3[0]] / N3 *
                scale;
        }
    }
    fftw_destroy_plan(plan_P);
...
void rtfmm::LaplaceKernel::m2l_fft_precompute_advanced3(
    int P, Cells3 &cs, PeriodicInteractionMapM2L &m2l_map,
    PeriodicInteractionPairs &m2l_pairs)
{
    if (verbose)
        printf("m2l_fft_precompute_advanced3\n");
    int num_src = m2l_srcs.size();
    int num_tar = m2l_tars.size();
...
#pragma omp parallel for
    for (int i = 0; i < num_src; i++)
    {
        ...
        fftw_execute_dft_r2c(plan_Q, Q,
                             reinterpret_cast<fftw_complex *>(&buffer[0]));
        for (int f = 0; f < N_freq; f++)
        {
...
#pragma omp parallel for
    for (int i = 0; i < num_tar; i++)
    {
        ...
        fftw_execute_dft_c2r(
            plan_P, reinterpret_cast<fftw_complex *>(&buffer[0]), p_fft_grid);
        auto m2l = m2l_map.begin();
        std::advance(m2l, i);
        Cell3 &cell_tar = cs[m2l->first];
        real scale = std::pow(cell_tar.depth > 0 ? 2 : 3, cell_tar.depth);
...
void rtfmm::LaplaceKernel::hadamard_8x8(
    int fft_size, conv_grid_setting &cgrid,
    std::vector<size_t> &interaction_count_offset_8x8,
    std::vector<size_t> &interaction_offset_f_8x8, AlignedVec &Qk_all,
    AlignedVec &Pk_all)
{
...
#pragma omp parallel for
    for (int i = 0; i < 26; i++)
    {
        ...
        {
            Qk_ptrs[i * BLOCK_SIZE + j] =
                &Qk_all[interaction_offset_f_8x8[2 * (offset0 + j) + 0]];
            Pk_ptrs[i * BLOCK_SIZE + j] =
                &Pk_all[interaction_offset_f_8x8[2 * (offset0 + j) + 1]];
        }
        ...
        for (int ipos = 0; ipos < 26; ipos++)
        {
            ...
            for (size_t j = 0; j < num_interaction; j += 2)
            {
                ...
                real *Qk1 =
                    Qk_ptr[j + 0] +
                    f * MAX_CHILD_NUM *
                        2; // Qk of children in frequency f, size = 8 * 2
                real *Qk2 = Qk_ptr[j + 1] + f * MAX_CHILD_NUM * 2;
                real *Pk1 =
                    Pk_ptr[j + 0] +
                    f * MAX_CHILD_NUM *
                        2; // Pk of children in frequency f, size = 8 * 2
            ...
            /*for(size_t j = 0; j < count; j++)
            {
                ...
                //c2c_8x8x1(Gk, Qk, Pk);
                c2c_8x8x1_naive(Gk, Qk, Pk);  // for no-AVX case, this is most
            efficient
            }*/
        }
    }
...
void rtfmm::LaplaceKernel::hadamard_1x27(
    ...
    std::vector<size_t> &interaction_offset_f_1x27, AlignedVec &Qk_all,
    AlignedVec &Pk_all)
{
    const int MAX_CHILD_NUM = 8;
...
#pragma omp parallel for
    for (int i = 0; i < 26; i++)
    {
        ...
        {
            Qk_ptrs[i * BLOCK_SIZE + j] =
                &Qk_all[interaction_offset_f_1x27[2 * offset0 + 2 * j + 0]];
            Pk_ptrs[i * BLOCK_SIZE + j] =
                &Pk_all[interaction_offset_f_1x27[2 * offset0 + 2 * j + 1]];
        }
    }
#pragma omp parallel for
    for (int f = 0; f < cgrid.N_freq; f++)
    {
        ...
        {
            size_t offset0 =
                (i == 0 ? 0 : interaction_count_offset_1x27[i - 1]);
            size_t offset1 = interaction_count_offset_1x27[i];
            size_t num_interaction = offset1 - offset0;
...
void rtfmm::LaplaceKernel::m2l_fft_precompute_t(int P, Cells3 &cs,
                                                PeriodicM2LMap &m2l_parent_map)
{
    ...
    if (verbose)
        printf("num_src_cell = %d, num_tar_cell = %d\n", num_src_cell,
               num_tar_cell);
    ...
    for (int i = 0; i < tar_cell_idxs.size(); i++)
    {
        ...
        for (int j = 0; j < m2l_list.size(); j++)
        {
            ...
            vec3r rv =
                (src_cell.x + src_offset - tar_cell.x) / (tar_cell.r * 2);
            vec3i rvi(std::round(rv[0]), std::round(rv[1]), std::round(rv[2]));
            int octant = rel2idx_map[hash(rvi)];
            m2l_parent_octant_map[tar_idx][octant] = {
                src_idx, is_image}; // cell located at tar's octant direction is
                                    // src(or -1)
        }
    }
    ...
    for (int k = 0; k < 26; k++)
    {
        for (int i = 0; i < tar_cell_idxs.size(); i++)
        {
            ...
            if (src_idx != -1)
            {
                ...
                {
                    interaction_offset_f_8x8.push_back(src_cell_local_idx *
                                                       fft_size);
                ...
                {
                    interaction_offset_f_1x27.push_back(src_cell_local_idx *
                                                        fft_size);
    ...
    for (int k = 0; k < num_surf; k++)
    {
        rtfmm::vec3i idx3 =
            surf_conv_map[k] +
            vec3i(P - 1, P - 1,
                  P - 1); // vec3i(P-1,P-1,P-1) is conv offset, see test_fft.cpp
        int idx = idx3[2] * cgrid.N * cgrid.N + idx3[1] * cgrid.N + idx3[0];
...
/* According to Fourier transformation addition theorem, interaction from
 * different srcs are added up in Fourier space, so we need set Pk to zero in
 * advance. */
#pragma omp parallel for
    ...
    fftw_plan plan_up = fftw_plan_many_dft_r2c(
        3, dim, MAX_CHILD_NUM, (real *)&fftw_in[0], nullptr, 1, cgrid.N3,
        (fftw_complex *)&fftw_out[0], nullptr, 1, cgrid.N_freq, FFTW_ESTIMATE);
#pragma omp parallel for
    for (int isrc = 0; isrc < num_src_cell; isrc++)
    {
        ...
        for (int k = 0; k < num_surf; k++)
        {
            ...
            for (int i = 0; i < cp.crange.number; i++)
            {
                ...
                octant = (octant == 13
                              ? 0
                              : octant); // for an image cell, it has only one
                                         // child, so we store it in position 0
                Qk_children[octant * cgrid.N3 + idx] =
                    Q_children[i * num_surf + k]; // insert Q into conv grid
            }
        }
        ...
        for (int f = 0; f < cgrid.N_freq; f++)
        {
            ...
            {
                // since FFTW's output is MAX_CHILD_NUM by Nfreq, we should
                // transpose it to Nfreq by MAX_CHILD_NUM for faster hadamard
                // product
                Qk_children[f * 2 * MAX_CHILD_NUM + 2 * j + 0] =
                    buffer[j * 2 * cgrid.N_freq + 2 * f + 0];
                Qk_children[f * 2 * MAX_CHILD_NUM + 2 * j + 1] =
                    buffer[j * 2 * cgrid.N_freq + 2 * f + 1];
            }
        }
    }
    ...
    {
        tbegin(hadamard_8x8);
        hadamard_8x8(fft_size, cgrid, interaction_count_offset_8x8,
                     interaction_offset_f_8x8, Qk_all, Pk_all);
        tend(hadamard_8x8);
    }
    if (interaction_cnt_1x27 > 0)
    {
        tbegin(hadamard_1x27);
        hadamard_1x27(fft_size, cgrid, interaction_count_offset_1x27,
                      interaction_offset_f_1x27, Qk_all, Pk_all);
        tend(hadamard_1x27);
    }
    ...
    fftw_plan plan_down = fftw_plan_many_dft_c2r(
        3, dim, MAX_CHILD_NUM, (fftw_complex *)&fftw_in2[0], nullptr, 1,
        cgrid.N_freq, (real *)&fftw_out2[0], nullptr, 1, cgrid.N3,
        FFTW_ESTIMATE);
#pragma omp parallel for
    for (int i = 0; i < num_tar_cell; i++)
    {
        ...
        for (int f = 0; f < cgrid.N_freq; f++)
        {
            for (int j = 0; j < MAX_CHILD_NUM; j++)
            {
                // since FFTW need frequent-first order, we should transpose Pk
                // here
                buffer0[j * 2 * cgrid.N_freq + 2 * f + 0] =
                    Pk_children[f * 2 * MAX_CHILD_NUM + 2 * j + 0];
                buffer0[j * 2 * cgrid.N_freq + 2 * f + 1] =
                    Pk_children[f * 2 * MAX_CHILD_NUM + 2 * j + 1];
            }
        }
        fftw_execute_dft_c2r(plan_down, (fftw_complex *)&buffer0[0],
                             (real *)&buffer1[0]);
        int tar_idx = tar_cell_idxs[i];
        Cell3 &ctar = cs[tar_idx];
        real scale = std::pow(
            ctar.depth >= -1 ? 2 : 3,
            ctar.depth +
                1); // since we store children's P, the depth should + 1
        for (int k = 0; k < num_surf; k++)
        {
            ...
            for (int j = 0; j < ctar.crange.number; j++)
            {
                ...
                octant =
                    (octant == 13
                         ? 0
                         : octant); // for an image cell, it has only one child,
                                    // so we extract it from position 0
                P_children[j * num_surf + k] +=
                    buffer1[cgrid.N3 * octant + idx] / cgrid.N3 * scale;
            }
        }
    }
...
void rtfmm::LaplaceKernel::l2l(int P, Cell3 &cell_parent, Cells3 &cs)
{
    ...
    Matrix pe2pc = get_p2p_matrix(x_equiv_parent, x_check_parent);
    Matrix q_equiv_parent =
        linear_equation_system_svd(pe2pc, cell_parent.p_check);
    ...
    for (int i = 0; i < cell_parent.crange.number; i++)
    {
        ...
        std::vector<rtfmm::vec3r> x_check_child =
            get_surface_points(P, cell_child.r * 1.05, cell_child.x);
...
void rtfmm::LaplaceKernel::l2l_precompute(int P, Cell3 &cell_parent, Cells3 &cs)
{
    ...
    {
        Cell3 &cell_child = cs[cell_parent.crange.offset + i];
        mat_vec_mul(matrix_l2l[cell_child.octant], cell_parent.p_check,
                    p_check_child);
        mat_mat_increment(cell_child.p_check, p_check_child);
    }
}

void rtfmm::LaplaceKernel::l2l_img_precompute(int P, Cell3 &cell_parent,
                                              Cells3 &cs)
{
    Cell3 &cell_child = cs[cell_parent.crange.offset];
...
void rtfmm::LaplaceKernel::l2p(int P, Bodies3 &bs_tar, Cell3 &cell_tar)
{
    /* get equivalent to check matrix */
    std::vector<rtfmm::vec3r> x_equiv =
        get_surface_points(P, cell_tar.r * 2.95, cell_tar.x);
    std::vector<rtfmm::vec3r> x_check =
        get_surface_points(P, cell_tar.r * 1.05, cell_tar.x);
    Matrix e2c = get_p2p_matrix(x_equiv, x_check);
...
void rtfmm::LaplaceKernel::l2p_precompute(int P, Cell3 &cell_tar)
{
    ...
    Matrix &q_equiv =
        cell_tar.p_check; // as in p2m, we can use p_check as buffer here
    ...
    for (int j = 0; j < cell_tar.bodies.size(); j += 4)
    {
        ...
        for (int idx = 0; idx < 4 - padding; idx++)
        {
            xj[idx] = cell_tar.bodies[j + idx].x[0];
            yj[idx] = cell_tar.bodies[j + idx].x[1];
            zj[idx] = cell_tar.bodies[j + idx].x[2];
        }
        ...
        for (int idx = 0; idx < 4 - padding; idx++)
        {
            if (verbose)
            {
                ...
                {
                    std::cout << "L2P cell " << cell_tar.idx << " has body "
                              << cell_tar.bodies[j + idx].q << ", "
                              << cell_tar.weights[j + idx] << std::endl;
                }
            }
            cell_tar.bodies[j + idx].p += cell_tar.weights[j + idx] * pj[idx];
            cell_tar.bodies[j + idx].f[0] -=
                cell_tar.weights[j + idx] * fxj[idx];
            cell_tar.bodies[j + idx].f[1] -=
                cell_tar.weights[j + idx] * fyj[idx];
            cell_tar.bodies[j + idx].f[2] -=
                cell_tar.weights[j + idx] * fzj[idx];
        }
    }
}

void rtfmm::LaplaceKernel::m2p(int P, Bodies3 &bs_tar, Cell3 &cell_src,
                               Cell3 &cell_tar, vec3r offset)
{
    std::vector<rtfmm::vec3r> x_equiv_src =
        get_surface_points(P, cell_src.r * 1.05, cell_src.x + offset);
    std::vector<vec3r> x_tar = get_bodies_x(bs_tar, cell_tar.brange);
...
void rtfmm::LaplaceKernel::p2l(int P, Bodies3 &bs_src, Cell3 &cell_src,
                               Cell3 &cell_tar, vec3r offset)
{
    std::vector<vec3r> x_src = get_bodies_x(bs_src, cell_src.brange, offset);
    Matrix q_src = get_bodies_q(bs_src, cell_src.brange);
    std::vector<rtfmm::vec3r> x_check_tar =
        get_surface_points(P, cell_tar.r * 1.05, cell_tar.x);
...
rtfmm::Matrix rtfmm::LaplaceKernel::get_p2p_matrix(std::vector<vec3r> &x_src,
...
rtfmm::Matriv rtfmm::LaplaceKernel::get_force_naive(std::vector<vec3r> &x_src,
                                                    std::vector<vec3r> &x_tar,
                                                    Matrix &q_src)
{
...
{
    if (verbose)
        std::cout << "dipole correction" << std::endl;
...
std::vector<rtfmm::real>
rtfmm::LaplaceKernel::get_G_matrix(std::vector<rtfmm::vec3r> &grid, int N)
{
...
std::vector<rtfmm::real>
rtfmm::LaplaceKernel::get_Q_matrix(Matrix &surface_q, int N,
                                   std::map<int, rtfmm::vec3i> &surf_conv_map)
{
...
void rtfmm::LaplaceKernel::get_Q_matrix(
    real *Q, Matrix &surface_q, int N,
    std::map<int, rtfmm::vec3i> &surf_conv_map)
{
    int surface_number = surface_q.m * surface_q.n;
    for (int i = 0; i < surface_number; i++)
...
#pragma omp parallel for
    ...
    {
        vec3r offset_child =
            Tree::get_child_cell_x(vec3r(0, 0, 0), r0, octant, 0);
        std::vector<vec3r> x_equiv_child_up =
            get_surface_points(P, r0 / 2 * 1.05, offset_child);
        Matrix ce2pc_up_precompute =
            get_p2p_matrix(x_equiv_child_up, x_check_up_parent);
        Matrix m1 = mat_mat_mul(UT_m2m_precompute, ce2pc_up_precompute);
        matrix_m2m[octant] =
            mat_mat_mul(VSinv_m2m_precompute, m1); // (VSinv)(UTG)
    }
...
#pragma omp parallel for
        ...
        {
            vec3r offset_child =
                Tree::get_child_cell_x(vec3r(0, 0, 0), r0, octant, 1);
            std::vector<vec3r> x_equiv_child_up =
                get_surface_points(P, r0 / 3 * 1.05, offset_child);
            Matrix ce2pc_up_precompute =
                get_p2p_matrix(x_equiv_child_up, x_check_up_parent);

            Matrix m1 = mat_mat_mul(UT_m2m_precompute, ce2pc_up_precompute);
            matrix_m2m_img[octant] =
                mat_mat_mul(VSinv_m2m_precompute, m1); // (VSinv)(UTG)
        }
    }
...
#pragma omp parallel for
    ...
    VSinvUT_l2p_precompute =
        mat_mat_mul(VSinv_l2p_precompute, UT_l2p_precompute);
}

void rtfmm::LaplaceKernel::precompute_m2l(int P, real r0, Cells3 cs,
                                          ...
                                          int images)
{
    ...
    if (verbose)
        std::cout << "precompute M2L, " << conv_grid << std::endl;
    std::vector<real> G0(conv_grid.N3);
    std::vector<complexr> Gk0(conv_grid.N_freq);
    fftw_plan plan_G = fftw_plan_dft_r2c_3d(
        conv_grid.N, conv_grid.N, conv_grid.N, G0.data(),
        reinterpret_cast<fftw_complex *>(Gk0.data()), FFTW_ESTIMATE);
    int range = images >= 2 ? 4 : 3; // when images <=1, possible M2L range is
                                     // [-3,3], otherwise [-4,4]
    if (verbose)
    ...
    /*
        ...
        note, here is 'M2L pair', not 'M2l parent pair', therefore total number
       is at 316 instead of 26
    */
    std::map<int, int> m2l_gk_relx_idx_map = get_relx_idx_map(range, 2);
    int m2l_gk_relx_idx_map_cnt = m2l_gk_relx_idx_map.size();
    std::vector<std::vector<rtfmm::complexr>> m2l_Gks_ordered(
        ...
        for (int j = -range; j <= range; j++)
        {
            for (int i = -range; i <= range; i++)
            {
                if (std::abs(i) > 1 || std::abs(j) > 1 || std::abs(k) > 1)
                {
                    ...
                    vec3r relative_pos = vec3r(i, j, k) * r0 *
                                         2; // relative_pos mean src's position
                    std::vector<rtfmm::vec3r> grid =
                        get_conv_grid(conv_grid.N, conv_grid.gsize,
                                      conv_grid.delta, relative_pos);
...
#pragma omp critical // std::map is not thread-safe
                    {
                        ...
                        // since the order for generating Gk is same as indices
                        // in m2l_gk_relx_idx_map, first generated Gk is stored
                        // in [0], second Gk in [1], ... so it is called
                        // m2l_Gks_'ordered'
    ...
    /*
        Q : What happened behind
       m2l_Gks_ordered[m2l_gk_relx_idx_map[hash(relx)]] ? A : relx -> index ->
       Gk
    */

    /*
        Precomputation for M2L_precompute_t
        For two neighbour cells, generate(store) interaction matrix Gk for their
       children. Since each cell has at most 8 child for nonperiodic M2L, 8*8=64
       matrix(real Gk or dummy matrix) will be stored for each neighbour
       pattern. Also, since there are at most 26 patterns of neighouring, the
       total # of matrices stored in ccGks is 26 * 64.
    */
    std::vector<vec3r> src_par_rels = get_relx<real>(
        1, 1); // all possible relative coordinates for any M2L pair's parents
    int m2l_par_rel_num =
        src_par_rels
            .size(); // # of neighbouring patterns, 26 for nonperiodic M2L
...
#pragma omp parallel for
    ...
    if (images >= 2)
    {
        ccGks_1x27.resize(
            m2l_par_rel_num,
            AlignedVec(1 * 28 * conv_grid.N_freq *
                       2)); // 27 padding 1 to 28 for simd(28 /4 = 7)
#pragma omp parallel for
        for (int i = 0; i < m2l_par_rel_num; i++)
        {
            ...
            for (int pz = -1; pz <= 1; pz++)
            {
                for (int py = -1; py <= 1; py++)
                {
                    for (int px = -1; px <= 1; px++)
                    {
                        ...
                        int hv = hash(src_xi);
                        if (m2l_gk_relx_idx_map.find(hv) !=
                            m2l_gk_relx_idx_map.end())
                        {
                            int gk_idx = m2l_gk_relx_idx_map[hv];
                            for (int f = 0; f < conv_grid.N_freq; f++)
                            {
                                ...
                                ccGks_1x27[i][idx + 0] =
                                    m2l_Gks_ordered[gk_idx][f].r;
                                ccGks_1x27[i][idx + 1] =
                                    m2l_Gks_ordered[gk_idx][f].i;
                            }
                        }
                        else
                        {
                            assert_exit(
                                false, "precompute_M2L error : cannot find gk");
                        }
                        cnt++;
                    }
                }
