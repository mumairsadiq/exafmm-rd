        ...
        for (int k = 0; k < 4 - padding; k++)
        {
            if ((btar + k)->idx == 0)
            {
                printf("P2P body %d, cell %d, r=%.4f, p2p_size=%ld, w*p=%.12f "
                       "* %.12f = %.12f\n",
                       0, cell_tar.idx, cell_tar.r, p2ps.size(), *(wtar + k), pj[k], *(wtar + k) * pj[k]);
            }
...
void rtfmm::LaplaceKernel::hadamard_8x8(int fft_size, conv_grid_setting &cgrid, std::vector<size_t> &interaction_count_offset_8x8, std::vector<size_t> &interaction_offset_f_8x8,
                                        AlignedVec &Qk_all, AlignedVec &Pk_all)
{
...
void rtfmm::LaplaceKernel::hadamard_1x27(int fft_size, conv_grid_setting &cgrid, std::vector<size_t> &interaction_count_offset_1x27, std::vector<size_t> &interaction_offset_f_1x27,
                                         AlignedVec &Qk_all, AlignedVec &Pk_all)
{
    const int MAX_CHILD_NUM = 8;
...
void rtfmm::LaplaceKernel::m2l_fft_precompute_t(int P, Cells3 &cs, PeriodicM2LMap &m2l_parent_map)
{
    ...
    for (int i = 0; i < tar_cell_idxs.size(); i++)
    {
        ...
        for (int j = 0; j < m2l_list.size(); j++)
        {
            ...
            m2l_parent_octant_map[tar_idx][octant] = {src_idx, is_image}; // cell located at tar's octant direction is
                                                                          // src(or -1)
        }
    }
    ...
    for (int k = 0; k < num_surf; k++)
    {
        rtfmm::vec3i idx3 = surf_conv_map[k] + vec3i(P - 1, P - 1,
...
#pragma omp parallel for
    ...
    fftw_plan plan_up =
        fftw_plan_many_dft_r2c(3, dim, MAX_CHILD_NUM, (real *)&fftw_in[0], nullptr, 1, cgrid.N3, (fftw_complex *)&fftw_out[0], nullptr, 1, cgrid.N_freq, FFTW_ESTIMATE);
#pragma omp parallel for
    for (int isrc = 0; isrc < num_src_cell; isrc++)
    {
        ...
        for (int k = 0; k < num_surf; k++)
        {
            ...
            for (int i = 0; i < cp.crange.number; i++)
            {
                ...
                octant = (octant == 13 ? 0 : octant);                                // for an image cell, it has only one
    ...
    fftw_plan plan_down =
        fftw_plan_many_dft_c2r(3, dim, MAX_CHILD_NUM, (fftw_complex *)&fftw_in2[0], nullptr, 1, cgrid.N_freq, (real *)&fftw_out2[0], nullptr, 1, cgrid.N3, FFTW_ESTIMATE);
#pragma omp parallel for
    for (int i = 0; i < num_tar_cell; i++)
    {
        ...
        int tar_idx = tar_cell_idxs[i];
        Cell3 &ctar = cs[tar_idx];
        real scale = std::pow(ctar.depth >= -1 ? 2 : 3,
                              ctar.depth + 1); // since we store children's P, the depth should + 1
        for (int k = 0; k < num_surf; k++)
        {
            ...
            for (int j = 0; j < ctar.crange.number; j++)
            {
                ...
                octant = (octant == 13 ? 0 : octant); // for an image cell, it has only one child,
...
void rtfmm::LaplaceKernel::l2p_precompute(int P, Cell3 &cell_tar)
{
    ...
    for (int j = 0; j < cell_tar.bodies.size(); j += 4)
    {
        ...
        for (int idx = 0; idx < 4 - padding; idx++)
        {
            ...
            cell_tar.bodies[j + idx].f[1] -= cell_tar.weights[j + idx] * fyj[idx];
            cell_tar.bodies[j + idx].f[2] -= cell_tar.weights[j + idx] * fzj[idx];
        }
    }
}
...
rtfmm::Matrix rtfmm::LaplaceKernel::get_p2p_matrix(std::vector<vec3r> &x_src, std::vector<vec3r> &x_tar)
{
...
rtfmm::Matriv rtfmm::LaplaceKernel::get_force_naive(std::vector<vec3r> &x_src, std::vector<vec3r> &x_tar, Matrix &q_src)
{
...
#pragma omp parallel for
    ...
    if (images >= 2)
    {
        ccGks_1x27.resize(m2l_par_rel_num,
                          AlignedVec(1 * 28 * conv_grid.N_freq * 2)); // 27 padding 1 to 28 for simd(28 /4 = 7)
#pragma omp parallel for
        for (int i = 0; i < m2l_par_rel_num; i++)
        {
