...
rtfmm::Bodies3 rtfmm::LaplaceFMM::solve()
{
    ...
    if (verbose)
        std::cout << "# of cells = " << cs.size() << std::endl;
    if (verbose)
        std::cout << "tree_depth_range = " << tree_depth_range << std::endl;
    tend(traverse);
...
void rtfmm::LaplaceFMM::M2L()
{
    ...
    // traverser.get_pairs(OperatorType::M2L); PeriodicInteractionMapM2L m2l_map
    // = traverser.get_map(OperatorType::M2L);
    PeriodicM2LMap m2l_parent_map = traverser.get_M2L_parent_map();
    // PeriodicInteractionMapM2L m2l_map2 =
    // traverser.get_m2l_map_from_m2l_parent_map();
    if (args.use_precompute)
    {
...
#pragma omp parallel for
        for (int leaf_idx = 0; leaf_idx < leaves.size(); leaf_idx++)
        {
            ...
            for (int body_idx = cell.brange.offset; body_idx < cell.brange.offset + cell.brange.number; body_idx++)
            {
                ...
                const vec3r dx_simcenter = (body.x - args.x).abs() + args.rega;
...
#pragma omp parallel for
        for (int leaf_idx = 0; leaf_idx < leaves.size(); leaf_idx++)
        {
            ...
            // find out bodies from adjacent cells that are influencing this
            // cell
...
void rtfmm::LaplaceFMM::check_traverser(Traverser &traverser)
{
    ...
    if (verbose)
        std::cout << "sum# : " << m2l_num + m2p_num + p2l_num << std::endl;
