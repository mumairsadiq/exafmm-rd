...
{
    if (verbose)
        printf("horizontal_periodic_near\n");
    /* images == 1 */
    Cell3 c;
    c.idx = cells.size();
    c.depth = -1;
...
{
    if (verbose)
        printf("horizontal_periodic_far\n");
    /* images >= 2 */
                    ...
                    for (int cz = -1; cz <= 1; cz++)
                    {
                        for (int cy = -1; cy <= 1; cy++)
                        {
                            for (int cx = -1; cx <= 1; cx++)
                            {
                                ...
                                int oy = py * 3 + cy;
                                int oz = pz * 3 + cz;
                                // M2L_pairs.push_back(make_pair(0,icell_idx,vec3r(ox,oy,oz)
                                // * cycle));
...
#pragma omp parallel for
    for (int j = 0; j < cells.size(); j++)
    {
        Cell3 &ctar = cells[j];
        if (ctar.depth == -1)
            continue; // the largest cell always neighbour with its 1st periodic
                      // images, so we need to exclude it
        for (int i = 0; i < cells.size(); i++)
        {
            Cell3 &csrc = cells[i];
            for (int pz = -1; pz <= 1; pz++)
...
int rtfmm::Traverser::adjacent(int a, int b, vec3r offset)
{
    ...
    real dist = (ca.r + cb.r) * 1.001; // warning : DO NOT ignore the float
                                       // error
...
rtfmm::PeriodicInteractionPairs rtfmm::Traverser::get_pairs(OperatorType type)
{
    switch (type)
    {
    case OperatorType::M2L:
        return M2L_pairs;
        break;
    case OperatorType::M2P:
        return M2P_pairs;
        break;
    case OperatorType::P2L:
        return P2L_pairs;
        break;
    default:
        return PeriodicInteractionPairs();
    }
}
