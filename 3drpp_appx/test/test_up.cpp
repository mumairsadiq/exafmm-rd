...
std::vector<int> generate_surf2conv_up(int p)
{
    ...
    {
        map[i] = (int)(p - 1 - surf[i * 3]) + ((int)(p - 1 - surf[i * 3 + 1])) * n1 + ((int)(p - 1 - surf[i * 3 + 2])) * n1 * n1;
    }
    return map;
}
...
void fft_up_equiv(RealVec &all_up_equiv, AlignedVec &fft_in, int p)
{
    ...
    int dim[3] = {n1, n1, n1};
    fftw_plan plan = fftw_plan_many_dft_r2c(3, dim, NCHILD, (real_t *)&fftw_in[0], nullptr, 1, nconv_, (fftw_complex *)(&fftw_out[0]), nullptr, 1, nfreq_, FFTW_ESTIMATE);
    TIME_END(up_prepare);
    TIME_BEGIN(up_core);
