...
class LaplaceKernel
{
  public:
    ...
    Matrix get_p2p_matrix(std::vector<vec3r> &x_src, std::vector<vec3r> &x_tar);

    Matriv get_force_naive(std::vector<vec3r> &x_src, std::vector<vec3r> &x_tar, Matrix &q_src);
     ...
     */
    template <typename T> static std::vector<vec<3, T>> get_relx(int range, int minv)
    {
  ...
  private:
    ...
    void hadamard_8x8(int fft_size, conv_grid_setting &cgrid, std::vector<size_t> &interaction_count_offset_8x8, std::vector<size_t> &interaction_offset_f_8x8, AlignedVec &Qk_all,
                      AlignedVec &Pk_all);

    void hadamard_1x27(int fft_size, conv_grid_setting &cgrid, std::vector<size_t> &interaction_count_offset_1x27, std::vector<size_t> &interaction_offset_f_1x27,
