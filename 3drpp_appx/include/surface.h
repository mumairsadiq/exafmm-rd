...
inline int get_surface_point_num(int p) { return 6 * (p - 1) * (p - 1) + 2; }
...
struct conv_grid_setting
{
    ...
    {
        os << "conv_grid(N = " << conv_grid.N << ", N3 = " << conv_grid.N3 << ", N_freq = " << conv_grid.N_freq << ", delta = " << conv_grid.delta
           << ", gsize = " << conv_grid.gsize << ")";
        return os;
    }
};
