...
#define TIME_END(a)                                                                                                                                                                \
    auto time_end_##a = std::chrono::high_resolution_clock::now();                                                                                                                 \
...
inline void title(std::string s) { std::cout << "<" << s << ">\n\n"; }
...
template <int N, typename T> class vec
{
  ...
  public:
    ...
    __host__ __device__ vec(T v0, T v1)
    {
        static_assert(N == 2);
        data[0] = v0;
        data[1] = v1;
    }
    __host__ __device__ vec(T v0, T v1, T v2)
    {
        static_assert(N == 3);
        data[0] = v0;
        data[1] = v1;
    ...
    __host__ __device__ vec operator+(const T v) const { return vec(*this) += v; }
    __host__ __device__ vec operator-(const T v) const { return vec(*this) -= v; }
    ...
    __host__ __device__ vec operator*(const T v) const { return vec(*this) *= v; }
    __host__ __device__ vec operator/(const T v) const { return vec(*this) /= v; }
    __host__ __device__ vec operator+(const vec v) const { return vec(*this) += v; }
    __host__ __device__ vec operator-(const vec v) const { return vec(*this) -= v; }
    __host__ __device__ vec operator*(const vec v) const { return vec(*this) *= v; }
    __host__ __device__ vec operator/(const vec v) const { return vec(*this) /= v; }
    __host__ __device__ const vec &operator+=(const T v)
    {
        for (int i = 0; i < N; i++)
        {
            this->data[i] += v;
    ...
    __host__ __device__ const vec &operator+=(const vec v)
    {
        for (int i = 0; i < N; i++)
        {
    ...
    __host__ __device__ bool operator==(const vec v)
    {
        for (int i = 0; i < N; i++)
        {
    ...
    __host__ __device__ bool operator!=(const vec v) { return !(*this == v); }
    __host__ __device__ friend vec operator*(const T v, const vec &_vec) // value * this
    {
        vec res;
        for (int i = 0; i < N; i++)
        {
    ...
    __host__ __device__ T r() { return sqrt(norm()); }
    T sum()
    {
        T res = 0;
        for (int i = 0; i < N; i++)
...
template <> class vec<4, double>
{
  ...
  public:
    vec() {}
    vec(double v) { d = _mm256_set1_pd(v); }
    ...
    vec(const __m256d v) { d = v; }
    vec(const vec &v) { d = v.d; }
    double &operator[](int i) { return data[i]; }
    const double &operator[](int i) const { return data[i]; }
    vec operator+(const double &v) const { return vec(*this) += v; }
    vec operator-(const double &v) const { return vec(*this) -= v; }
    vec operator-() const { return vec(_mm256_sub_pd(_mm256_set1_pd(0), d)); }
    vec operator*(const double &v) const { return vec(*this) *= v; }
    vec operator/(const double &v) const { return vec(*this) /= v; }
    ...
    vec operator==(const vec &v) { return vec(_mm256_cmp_pd(d, v.d, _CMP_EQ_OQ)); }
    vec operator>(const vec &v) { return vec(_mm256_cmp_pd(d, v.d, _CMP_GT_OQ)); }
    vec operator>=(const vec &v) { return vec(_mm256_cmp_pd(d, v.d, _CMP_GE_OQ)); }
    vec operator<(const vec &v) { return vec(_mm256_cmp_pd(d, v.d, _CMP_LT_OQ)); }
    vec operator<=(const vec &v) { return vec(_mm256_cmp_pd(d, v.d, _CMP_LE_OQ)); }
    vec operator!=(const vec &v) { return vec(_mm256_cmp_pd(d, v.d, _CMP_NEQ_OQ)); }
    ...
    vec sqrt() { return vec(_mm256_sqrt_pd(d)); }
};
