...
int rtfmm::Traverser::adjacent(int a, int b)
{
    Cell3 ca = cells[a];
    Cell3 cb = cells[b];
    vec3r dx = (ca.x - cb.x).abs();
    real dist = ca.r + cb.r;
    return dx[0] <= dist && dx[1] <= dist && dx[2] <= dist;
}

int rtfmm::Traverser::neighbour(int a, int b)
{
    return adjacent(a,b) && cells[a].depth == cells[b].depth;
}
