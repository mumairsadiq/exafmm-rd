...
if len(sys.argv) > 1:
    n = int(sys.argv[1])
p, L2p_exafmmt, L2f_exafmmt, L2kt_exafmmt, L2tt_exafmmt = get_exafmmt_data(n)
p2, L2p_rtfmmt, L2f_rtfmmt, L2kt_rtfmmt, L2tt_rtfmmt = get_rtfmm_data(n)
...
plt.ylabel('L2-err')
plt.legend()
plt.savefig("compare_P_L2.png", dpi=300)
...
plt.legend()
plt.savefig("compare_P_kernel-time.png", dpi=300)
...
plt.legend()
plt.savefig("compare_P_total-time.png", dpi=300)